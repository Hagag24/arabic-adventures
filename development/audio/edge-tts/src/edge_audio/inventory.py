...
def get_inventory() -> List[Dict[str, Any]]:
    ...
    res = subprocess.check_output(
        ["pnpm.cmd", "exec", "tsx", "-e", script], cwd=str(config.PROJECT_ROOT)
    )
    ...
    for item in raw_inventory:
        ...
        staging_mp3_path = str(config.STAGING_MP3_DIR / f"{rel_path}.mp3")
        staging_wav_path = str(config.STAGING_CATALOG_DIR / f"{rel_path}.wav")
        ...
        inventory.append(
            {
                ...
                "wavPath": wav_path,
                "stagingMp3Path": staging_mp3_path,
                "stagingWavPath": staging_wav_path,

...
interface JourneyResultPageProps {
  params: Promise<{
    journeySlug: string;
  }>;
}

export default async function JourneyResultPage({
  params,
}: JourneyResultPageProps) {
  ...
  // Ensure journey is completed before showing badge
  ...
  if (journey.themeKey === "ancient-egypt") {
    ...
    badgeAccent = "bg-rose-500";
  }

  return (
    <div
      className={`flex flex-col min-h-screen bg-gradient-to-br ${bgGradient} text-right dir-rtl items-center justify-center p-6`}
    >
      ...
      >
        ...
        <p className="text-sm md:text-base text-slate-600 leading-relaxed mb-8 max-w-sm mx-auto">
          تهانينا الحارة لك يا بطل! لقد أنهيت مغامرة{" "}
          <strong className="text-slate-800">«{journey.title}»</strong> بكافة
          تحدياتها ونلت هذا اللقب بجدارة لاستماعك وتفكيرك الذكي.
        </p>
